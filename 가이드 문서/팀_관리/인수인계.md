...
>
> 다른 가이드:
> - **프로젝트_총괄/빌드_실행.md** - 빌드/실행
> - **프로젝트_총괄/프로젝트_개요.md** - 입문자용 큰 그림
> - **프로젝트_총괄/코드_구조.md** - 기술 상세
> - **NPC_트레이너/NPC_추가_가이드.md** - NPC/트레이너 추가 매뉴얼
> - **팀_관리/변경_이력.md** - 최신 변경 이력
...
### 인게임 메뉴 시스템 (M 키)
- `Scene::INGAME_MENU` 씬 + `InGameMenuState` enum 추가
- 포켓몬/가방(스텁)/저장 메뉴 구조 구현
- 포켓몬 상세 화면: 기술 타입/위력/명중 + 타입 상성(약점/저항/무효) 표시
- 상세 설명: `시스템_구현/인게임_메뉴.md`

### NPC 일회성 이벤트 시스템
- `NpcTag` enum + `NpcDef::tag` 필드 추가
- `isNpcHidden()`: 스타터 포켓볼 숨김, 블루 사라짐
- `overrideLines_` / `startNpcDialog()`: 동적 대사 오버라이드
- 소포 퀘스트 플래그: `gotParcel`, `deliveredParcel`, `rivalLabTalked`
- 상세 설명: `시스템_구현/NPC_이벤트.md`
...
| 항목 | 상태 |
|------|------|
| 가방 시스템 (포션, 몬스터볼 등) | 미구현 (스텁 메뉴만 있음) |
| 야생 포켓몬 포획 | 미구현 |
| 상태이상 (마비, 독, 잠 등) | 미구현 |
| HM 기술 (베기, 파도타기 등) | 풀베기만 골격 구현 |
| 메뉴 (M 키) | M키 메뉴 구현 완료 (INGAME_MENU 씬) |
| 세이브/로드 | 미구현 |
...
4. **저장/로드** - 파일 1개로 게임 상태 직렬화
5. **BGM 실제 재생** - winmm `mciSendString` 으로 wav/mid 재생
6. **사운드 효과** - 메뉴 클릭, 배틀 사운드 등
7. **미구현 건물 채우기** - School/Speech/Nickname/Museum 등
8. **HM01 베기** - 가방 아이템으로 HM01 추가 (현재는 풀베기만 구현됨)
9. **트레이너 대각선/시야 차단** - 시야 검사 고도화
...
문서들이 다른 각도에서 같은 시스템을 설명하므로,
이해 안 되면 다른 문서를 보는 것도 방법:
- `프로젝트_총괄/프로젝트_개요.md` - 입문자용 큰 그림
- `프로젝트_총괄/코드_구조.md` - 기술 상세
- `NPC_트레이너/NPC_추가_가이드.md` - NPC/트레이너 작업
- `팀_관리/변경_이력.md` - 최신 변경 사항
